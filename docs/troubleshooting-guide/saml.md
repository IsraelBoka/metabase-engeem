...
Verify that the Single Sign On URL (or equivalent) that you enter on your SAML provider’s website has `/auth/sso/` appended to it. For instance, if you want your users to end up at `https://data.engeem.com.mycompany.com`, the full URL should be `https://data.engeem.com.mycompany.com/auth/sso/`.
